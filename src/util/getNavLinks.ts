...
export async function getNavLinks(
  ...
  const links = (await getNav()).filter(
    (x) => !("header" in x) && x.slug
  ) as NavItem[];
...
export function getPreviousAndNext(
  ...
  const index = links.findIndex((x) =>
    Astro.url.pathname.replace(/\/$/, "").endsWith(x.slug)
  );

  const makeLinkItem = ({ text, slug }: NavItem): LinkItem => ({
    text,
    link: `/${slug}/`,
  });
